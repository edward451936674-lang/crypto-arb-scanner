...
from app.services.binance_pilot_readiness import build_binance_pilot_readiness_preview
...
@app.get("/api/v1/execution/account-state-preview")
async def get_execution_account_state_preview(
    symbols: str | None = Query(default=None, description="Comma separated base symbols, e.g. BTC,ETH,SOL"),
    route_keys: list[str] | None = Query(
        default=None,
        description="Repeat route_keys to filter preview to specific execution routes",
    ...
    candidate_models = [ExecutionCandidate.model_validate(item) for item in selected_candidates]
    config_snapshot = resolve_execution_account_state_config_snapshot(settings)
    decisions = evaluate_execution_account_state_decisions(candidates=candidate_models, config=config_snapshot)
    allowed_count = sum(1 for item in decisions if item.allowed)
    blocked_count = len(decisions) - allowed_count
...
@app.post("/api/v1/execution/dry-run-submit")
async def post_execution_dry_run_submit(
    payload: dict[str, object] | None = Body(default=None),
    symbols: str | None = Query(default=None, description="Comma separated base symbols, e.g. BTC,ETH,SOL"),
    route_keys: list[str] | None = Query(
        default=None,
...
@app.post("/api/v1/execution/binance-pilot-readiness-preview")
async def post_binance_pilot_readiness_preview(
    payload: dict[str, object] | None = Body(default=None),
    symbols: str | None = Query(default=None, description="Comma separated base symbols, e.g. BTC,ETH,SOL"),
    route_keys: list[str] | None = Query(default=None, description="Repeat route_keys to filter preview to specific execution routes"),
    top_n: int = Query(default=10, ge=1, le=500),
    only_actionable: bool = Query(default=False),
    include_test: bool = Query(default=False),
) -> dict[str, object]:
    ...
    preview = await build_binance_pilot_readiness_preview(
        candidates=[ExecutionCandidate.model_validate(item) for item in selected_candidates],
        settings=settings,
        request_arm_token=request_arm_token,
    )
    ...
    return {
        "candidate_count": len(candidates),
        "preview_count": len(selected_candidates),
        "preview_only": True,
        "is_live": False,
        **preview,
    }
