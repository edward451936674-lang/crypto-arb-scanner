...
class ArbitrageScannerService:
    ...
    def _build_pair_opportunity(
        ...
        if data_quality_status != "healthy":
            risk_flags.append("degraded_data_quality")
        if any("cross_exchange_price_outlier" in flag for flag in data_quality_flags):
            risk_flags.append("cross_exchange_price_quality_risk")
    ...
    def _conviction_score(
        ...
        if "abnormal_hourly_funding" in opportunity.risk_flags:
            score -= 0.10
        if "degraded_data_quality" in opportunity.risk_flags:
            score -= 0.06
    ...
    def _opportunity_data_quality(
        ...
        long_status = long_snapshot.data_quality_status or "healthy"
        short_status = short_snapshot.data_quality_status or "healthy"
        status = max((long_status, short_status), key=lambda value: DATA_QUALITY_SEVERITY.get(value, 1))

        long_score = long_snapshot.data_quality_score if long_snapshot.data_quality_score is not None else 1.0
        short_score = short_snapshot.data_quality_score if short_snapshot.data_quality_score is not None else 1.0
        score = min(long_score, short_score)

        flags = list(
            dict.fromkeys(
                (long_snapshot.data_quality_flags or [])
                + (short_snapshot.data_quality_flags or [])
            )
        )

        drivers: list[str] = []
        degraded_legs = sum(1 for leg_status in (long_status, short_status) if leg_status != "healthy")
        if degraded_legs == 0:
            drivers.append("both_legs_healthy")
        elif degraded_legs == 1:
            drivers.append("one_leg_degraded")
        else:
            drivers.append("both_legs_degraded")
        if "timestamp_stale" in flags:
            drivers.append("timestamp_stale")
        if any("cross_exchange_price_outlier" in flag for flag in flags):
            drivers.append("degraded_cross_exchange_price_signal")

        penalty = DATA_QUALITY_PENALTY.get(status, 0.85)
        return status, score, flags, drivers, penalty
    ...
    def _determine_execution_mode(
        ...
        if "degraded_data_quality" in risk_flags:
            gap_drivers.append("degraded_data_quality")
