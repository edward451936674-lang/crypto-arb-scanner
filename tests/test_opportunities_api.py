...
def test_degraded_quality_propagates_to_opportunity_risk_and_sizing() -> None:
    ...
    healthy_right_with_quality = healthy_right.model_copy(
        update={
            ...
            "data_quality_flags": ["insufficient_cross_exchange_peers"],
        }
    )
    ...
    assert "insufficient_cross_exchange_peers" in degraded_opportunity.data_quality_flags
...
def test_get_opportunities_mixed_collection_uses_only_gate_accepted(monkeypatch) -> None:
    ...
    assert statuses == {"binance": "healthy", "okx": "degraded"}
    assert all(status in {"healthy", "degraded"} for status in statuses.values())
