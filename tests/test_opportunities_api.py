...
def test_execution_account_input_builder_uses_settings_defaults() -> None:
    scanner = ArbitrageScannerService()
    opportunity = scanner.build_opportunities(
        [
            _snapshot("binance", 100.0, funding_rate=-0.001, funding_rate_source="latest_reported"),
            _snapshot("okx", 101.0, funding_rate=0.001, funding_rate_source="current"),
        ]
    ...
    settings = Settings(
        ...
        execution_live_target_leverage=1.8,
        execution_live_max_allowed_leverage=1.9,
        execution_live_required_liquidation_buffer_pct=29.0,
        execution_live_remaining_total_cap_pct=0.21,
        execution_live_remaining_symbol_cap_pct=0.11,
        execution_live_remaining_long_exchange_cap_pct=0.12,
        execution_live_remaining_short_exchange_cap_pct=0.13,
    )

    inputs = build_execution_account_inputs(settings, opportunity)
    assert inputs.extended_size_up_enabled is False
    assert inputs.live_target_leverage == 1.8
    assert inputs.live_max_allowed_leverage == 1.9
    assert inputs.live_required_liquidation_buffer_pct == 29.0
    assert inputs.live_remaining_total_cap_pct == 0.21
    assert inputs.live_remaining_symbol_cap_pct == 0.11
    assert inputs.live_remaining_long_exchange_cap_pct == 0.12
    assert inputs.live_remaining_short_exchange_cap_pct == 0.13
