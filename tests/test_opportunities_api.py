...
def test_normal_candidate_has_positive_edge_buffer_and_high_normal_eligibility() -> None:
    scanner = ArbitrageScannerService()
    opportunities = scanner.build_opportunities(
        [
    ...
    item = opportunities[0]
    assert item.execution_mode == "normal"
    assert item.edge_buffer_bps >= 0
    assert item.normal_eligibility_score >= 0.70
    assert "positive_edge_buffer" in item.normal_eligibility_drivers
    assert "meets_normal_conviction" in item.normal_eligibility_drivers
    assert "meets_normal_funding_confidence" in item.normal_eligibility_drivers


def test_small_probe_route_exposes_negative_normal_eligibility_drivers() -> None:
    scanner = ArbitrageScannerService()
    opportunities = scanner.build_opportunities(
        [
            _snapshot("binance", 100.0, funding_rate_source="latest_reported"),
    ...
    assert item.execution_mode == "small_probe"
    assert item.edge_buffer_bps < 0
    assert "negative_edge_buffer" in item.normal_eligibility_drivers
    assert "positive_edge_buffer" not in item.normal_eligibility_drivers


def test_degraded_data_quality_adds_normal_required_edge_surcharge() -> None:
    scanner = ArbitrageScannerService()
    healthy = scanner.build_opportunities(
        [
            _snapshot("binance", 100.0, funding_rate=-0.001, funding_rate_source="latest_reported"),
            _snapshot("okx", 101.0, funding_rate=0.001, funding_rate_source="current"),
        ]
    ...
    degraded = scanner.build_opportunities(
        [
            degraded_long,
            _snapshot("okx", 101.0, funding_rate=0.001, funding_rate_source="current"),
        ]
    ...
    assert degraded.normal_required_edge_bps > healthy.normal_required_edge_bps
    assert degraded.normal_required_edge_bps == healthy.normal_required_edge_bps + 1.0
    assert degraded.edge_buffer_bps < healthy.edge_buffer_bps
    assert "degraded_data_quality" in degraded.normal_eligibility_drivers
